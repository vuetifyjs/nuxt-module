...
describe('supportsSsrConfigHmr', () => {
  it('exposes the documented floor', () => {
    expect(MIN_NUXT_VERSION_FOR_SSR_CONFIG_HMR).toBe('4.0.0')
  })

  it('accepts the validated Nuxt versions (>= 4.0.0)', () => {
    expect(supportsSsrConfigHmr('4.0.0')).toBe(true)
    expect(supportsSsrConfigHmr('4.2.5')).toBe(true)
  ...
  it('rejects older Nuxt (restart fallback)', () => {
    ...
    expect(supportsSsrConfigHmr('4.0.0-rc.1')).toBe(false)
  })
